...
export default function BlogPost(props) {
  ...
  const prosConsQuery = [
    {
      ...
      items: [
        ...
        "Easy-to-use themes, even you're not tech-savvy",
  ...
  const blogContent = documentToReactComponents(
      ...
      <Layout headerBg={"white"} headerIsStatic>
        <Helmet>
          ...
          <meta
            name="description"
            content={
              metaDescription ? metaDescription.metaDescription : intro.intro
            }
          />
          <meta
            property="og:url"
        ...
        <VevolSection backgroundColour={"white"}>
          <Container className="blog-content">
            <div className="blog-content__container">
              ...
              <>{blogContent}</>
              {title ===
                "The Complete Shopify Checklist for an easy Shopify Store setup" && (
                <div class="klaviyo-form-YAaFdq"></div>
              )}
            </div>
            <div
              className={`table-of-contents__progress-bar ${
                isTableOfContentsHidden
                  ? "table-of-contents__progress-bar--hidden"
                  : ""
              }`}
            >
              <ProgressBar />
              <TableOfContents
                ...
                content={blogContent
                  .filter(
                    (content) =>
                      content.props.tag === "h2" || content.props.tag === "h4"
                  )
                  .map((content) => {
          ...
          <Container className={"vm-section mt-6 pb-4"}>
            <ProsCons prosConsQuery={prosConsQuery} />
          </Container>
        ...
        <SplitNav
          leftTitle={
            previous ? previous.slug.replaceAll("-", " ") : "See all articles"
          }
          leftUrl={previous ? `/blog/${previous.slug}` : "/blog"}
          rightTitle={
            next ? next.slug.replaceAll("-", " ") : "See all articles"
          }
